 ...
 * Gateway API calls:
 * 
 *   URL: 				[server]:[port]/api/discovery/nodes/community/{commid}
 *   METHODS: 			GET
 ...
 *
 */
public class DiscoveryCommunityNodesCommid extends ServerResource {
	 ...
	 */
	private static final String ATTR_CID = "commid";
	
	// === OVERRIDEN HTTP METHODS ===
	
	/**
	 * Retrieves community nodes
	 * 
	...
	public Representation represent() {
		...
		return getCommunityNodes(attrCid, logger, config);
	}
	 ...
	 * @return list[{agid: string, cid: string, company: string}]
	 */
	private Representation getCommunityNodes(String commid, Logger logger, XMLConfiguration config){
		
		CommunicationManager communicationManager 
			= (CommunicationManager) getContext().getAttributes().get(Api.CONTEXT_COMMMANAGER);

		return communicationManager.getCommunityNodes(commid);
