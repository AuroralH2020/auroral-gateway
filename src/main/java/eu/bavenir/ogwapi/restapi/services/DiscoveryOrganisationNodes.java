 ...
 * Gateway API calls:
 * 
 *   URL: 				[server]:[port]/api/discovery/nodes/organisation
 *   METHODS: 			GET
 ...
 *
 */
public class DiscoveryOrganisationNodes extends ServerResource {
	
	// === OVERRIDEN HTTP METHODS ===
	
	/**
	 * Returns list of organisation nodes.
	 * 
	...
	public Representation represent() {
		...
		return getOrganisationNodes(logger, config);
	}
	 ...
	 * @return list[{agid: string, cid: string, company: string}]
	 */
	private Representation getOrganisationNodes(Logger logger, XMLConfiguration config){
		
		CommunicationManager communicationManager 
			= (CommunicationManager) getContext().getAttributes().get(Api.CONTEXT_COMMMANAGER);

		return communicationManager.getOrganisationNodes();
